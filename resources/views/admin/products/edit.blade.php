...
@section('script')
    <script>
        ...
        variations.forEach(variation => {
            const dtp1Instance = new mds.MdsPersianDateTimePicker(document.getElementById(
                `variationOnDateSaleto-${variation.id}`), {
                targetTextSelector: `#variationOnInputDateSaleto-${variation.id}`,
                enableTimePicker: true,
                textFormat: 'yyyy-MM-dd HH:mm:ss',

            });
            const dtp2Instance = new mds.MdsPersianDateTimePicker(document.getElementById(
                `variationOnDateSaleFrom-${variation.id}`), {
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select id="tagSelect" name="tag_ids[]" class="form-control" multiple data-live-search="true">
                            @php
                                $productTagsId = $product->tags()->pluck('id')->toArray();
                            @endphp
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}"
                                    {{ in_array($tag->id, $productTagsId) ? 'selected' : '' }}>{{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                                            ...
                                            <div class="input-group">
                                                <div class="input-group-prepend order-2">
                                                    <span class="input-group-text"
                                                        id="variationOnDateSaleFrom-{{ $variation->id }}">
                                                        <i class="fas fa-clock"></i>
                                                    </span>
                                                </div>
                                                <input type="text" class="form-control"
                                                    id="variationOnInputDateSaleFrom-{{ $variation->id }}"
                                                    name="variation_values[{{ $variation->id }}][date_on_sale_from]"
                                                    value="{{ $variation->date_on_sale_from ? verta($variation->date_on_sale_from)->format('Y-m-d') : '' }}"
>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group col-md-3">
                                            ...
                                            <div class="input-group">
                                                <div class="input-group-prepend order-2">
                                                    <span class="input-group-text"
                                                        id="variationOnDateSaleto-{{ $variation->id }}">
                                                        <i class="fas fa-clock"></i>
                                                    </span>
                                                </div>
                                                <input type="text" class="form-control"
                                                    id="variationOnInputDateSaleto-{{ $variation->id }}"
